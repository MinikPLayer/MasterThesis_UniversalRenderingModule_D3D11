...
struct WVPMatrix {
    ...
    void Apply(VertexConstantBuffer& buffer) const {
        buffer.worldViewPerspective = DirectX::XMMatrixTranspose(worldViewPerspective);
...
namespace {
    ...
    {
        // 1. World Matrix
        Matrix matScale = DirectX::XMMatrixScaling(modelScale.x, modelScale.y, modelScale.z);
        Matrix matRotation = DirectX::XMMatrixRotationRollPitchYaw(
        ...
        Matrix matTranslation = DirectX::XMMatrixTranslation(modelPosition.x, modelPosition.y, modelPosition.z);
    ...
    constexpr static const Vector3 CAM_POS = { 0.0f, 4.0f, -8.0f };
    constexpr static const Vector3 CAM_TARGET = { 0.0f, 0.0f, 0.0f };
    constexpr static const Vector3 CAM_UP = { 0.0f, 1.0f, 0.0f };
    ...
    constexpr const bool ENGINE_MODE = true;
    constexpr const bool ENGINE_LOOP_MODE = true;
    auto SelectedTest = std::unique_ptr<ITest>(new MultipleShadersTest());
    ...
    int ActualMainEngine(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow) {
        ...
        if (ENGINE_LOOP_MODE) {
            ...
            constexpr const float drawInterval = 1.0;
