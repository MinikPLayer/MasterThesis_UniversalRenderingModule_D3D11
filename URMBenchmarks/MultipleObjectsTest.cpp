...
void MultipleObjectsTest::AddObject() {
	...
	const auto targetObjectCount = 8000;

	float i = this->mCurrentObjectCount % targetObjectCount;
	auto iOffset = this->mCurrentObjectCount / targetObjectCount;

	i += 1.0f / (iOffset + 1);

	auto x = sin(i * 0.5f) * distance * i / targetObjectCount;
	auto z = cos(i * 0.5f) * distance * i / targetObjectCount;
	newObject->GetTransform().SetLocalPosition({ x, -0.9f, z });
